...
const onSongPlaying = () => {
    ...
    try {
        ...
        localStorage.setItem('playSongTime', audioPlayer.value!.currentTime.toString());
        SongManger.curTime = audioPlayer.value.currentTime;
    } catch (err) { }
};
...
const onPlayListSongClick = async (song: TTrackType) => {
    await audioPlayer.value!.play();
...
const isShowSongDetail = ref(false)
const showSongDetail = () => {
    isShowSongDetail.value = true
    GlobalVar.inSongDetail = true
    GlobalVar.addSongPlayClass("inSongDetail")
}
</script>
<template>
    ...
    <div :class="[...GlobalVar.songPlayClass, 'SongPlay']" v-show="hidePanel">

        <audio ref="audioPlayer" :src="initSongUrl" autoplay @ended="onAudioEnded" @timeupdate="onSongPlaying"></audio>

        <div class="left" @click.stop="showSongDetail">
            <div class="imgCover">
                <div class="imgBackground">
                    <img :src="SongManger.nowSong?.al.picUrl" :class="SongManger.isPlaying ? 'playing' : ''" alt="" />
                </div>
            </div>
            <div class="label">
                <n-ellipsis style="max-width: 123px">
                    <span :title="SongManger.nowSong?.name">{{ SongManger.nowSong?.name }}</span>
                </n-ellipsis>
            </div>
        </div>
        <div class="right">
            ...
            <div class="playIcon" @click="onPlayBtnClick">
                <n-progress class="songProgress" type="circle" :percentage="songPercentage" :show-indicator="false"
                            status="success" />
    ...
    <n-drawer v-model:show="showPlayList" style="width: 60vw; height: 100vh" placement="right"
              :on-after-enter="onAfterEnter" :on-after-leave="onAfterLeave">
    ...
    <SongDetail v-model="isShowSongDetail"></SongDetail>
